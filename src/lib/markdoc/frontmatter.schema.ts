...
export const label = z.optional(
  z.enum(['twitter', 'github', 'npm', 'dev.to', 'medium', 'youtube', 'podcast', 'deprecated', 'archived'])
);
...
export const notes = z.discriminatedUnion('external', [
  ...
  baseSchema.extend({
    external: z.literal(true),
    url: z.string({
      required_error: 'external is true but url is missing. url must be set for notes marked as external.',
      invalid_type_error: 'external should be string.',
    }),
  }),
]);
...
export const talk = z.discriminatedUnion('external', [
  ...
  baseSchema.extend({
    external: z.literal(true),
    url: z.string({
      required_error: 'external is true but url is missing. url must be set for talks marked as external.',
      invalid_type_error: 'external should be string.',
    }),
  }),
]);
